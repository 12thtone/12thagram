...
- (void)viewDidLoad
{
    ...
    [self.filterCollectionView registerClass:[ImagesCollectionViewCell class] forCellWithReuseIdentifier:@"cell"];
...
- (void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (invertFilter) {
            ...
            if (pixelFilter) {
                [pixelFilter setValue:result forKeyPath:kCIInputImageKey];
            }
            
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Concussion", @"Concussion Filter")];
        }
    }];
}
